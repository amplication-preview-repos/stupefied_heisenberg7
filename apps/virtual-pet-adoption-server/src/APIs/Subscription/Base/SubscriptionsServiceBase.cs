...
public abstract class SubscriptionsServiceBase : ISubscriptionsService
{
    ...
    public SubscriptionsServiceBase(VirtualPetAdoptionDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<Subscription> CreateSubscription(SubscriptionCreateInput createDto)
    {
        ...
        if (createDto.Pet != null)
        {
            subscription.Pet = await _context
                .Pets.Where(pet => createDto.Pet.Id == pet.Id)
                .FirstOrDefaultAsync();
        }
    ...
    )
    {
        ...
        {
            subscription.Pet = await _context
                .Pets.Where(pet => updateDto.Pet == pet.Id)
                .FirstOrDefaultAsync();
        }
    ...
    /// </summary>
    public async Task<Pet> GetPet(SubscriptionWhereUniqueInput uniqueId)
    {
        var subscription = await _context
            .Subscriptions.Where(subscription => subscription.Id == uniqueId.Id)
            .Include(subscription => subscription.Pet)
            .FirstOrDefaultAsync();
        if (subscription == null)
        {
            throw new NotFoundException();
        }
        return subscription.Pet.ToDto();
    }
